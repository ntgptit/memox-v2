...
| DT4 | folder import target loading must ignore active folder-detail search | root folder contains deck `Import target` and `folderChildrenToolbarStateProvider(root.id)` has search term `not matching` | `folderActionControllerProvider(root.id).loadImportDeckTargets()` is called | returned import targets still include deck `Import target` | C0+C1 |
...
| DT2 | import preview blocks invalid content, then valid commit refreshes list and resets import draft | import draft format is set to CSV, content first has missing back text, then valid CSV content | `preparePreview` runs for invalid content, then valid content, then `commitImport` runs | invalid preparation has one issue and cannot commit, valid preparation can commit, one card is inserted, and import draft is reset | C0+C1 |
