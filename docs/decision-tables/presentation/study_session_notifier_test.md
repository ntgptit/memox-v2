...
| DT12 | study session mutation must refresh cached Progress sessions | Progress active-session provider has already loaded from the same fake repo and Review batch submission succeeds | `answerCurrentReviewModeAsRemembered()` is called and Progress sessions are read again | fake repo active-session load count increases from one to two, proving the cached Progress provider was invalidated | C0+C1 |
...
| DT14 | match batch controller forwards the board item grade map | repo fake returns a New Study Match snapshot and records match batch calls | `answerCurrentMatchModeBatch({item-1: correct, item-2: incorrect})` is called | repo receives one match batch call with the exact item-grade map, the New Study mode list, and controller state has no error | C0+C1 |
