...
| DT3 | cancel toolbar action must ask for confirmation before mutating the session | study session screen is rendered with a cancellable in-progress session | user taps tooltip `Cancel` | `Cancel this session?` dialog appears and fake repo `cancelCount` remains zero | C0+C1 |
