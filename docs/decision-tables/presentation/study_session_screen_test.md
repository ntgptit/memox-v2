...
| DT7 | session is in progress and current mode is Match | snapshot has `StudyMode.match`, two current-round items, and shuffle answers disabled | session screen renders active Match branch | title `Match`, toolbar actions, progress `0%`, and a two-column board with backs on the left and fronts on the right are visible | C0+C1 |
| DT8 | Match tiles contain long definition text and short label text | snapshot has one long `back` value and two short `front` values in current round | Match branch renders the board | the long definition remains visible inside a match tile, short labels use the stable tile role, and no old `Skip card` control is shown | C0+C1 |
