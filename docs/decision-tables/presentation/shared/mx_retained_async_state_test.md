...
| DT4 | first load fails and the caller provides section-specific error chrome | `MxRetainedAsyncState` is built with `error=StateError('boom')`, no data, and an `errorBuilder` that returns `Custom error` | widget renders first-load failure | `Custom error` is visible and the default `MxErrorState` is absent | C0+C1 |
...
| DT3 | retained data exists inside a bounded-width parent while refresh overlay is active | parent constrains `MxRetainedAsyncState` to width `320` and retained data is loading | widget lays out retained content | retained content receives the parent width and `mx_retained_async_refresh_bar` remains visible | C0+C1 |
