...
| DT6 | constrained action surfaces preserve tappable content instead of overflowing | `MxEmptyState`, `MxFolderTile`, and `MxPrimaryButton` render inside narrow mobile host widths with icons, labels, and trailing action metadata | each constrained surface is pumped inside the MemoX theme wrapper | no RenderFlex overflow or Flutter exception is thrown, keyed controls remain visible, and labels stay discoverable for screen taps | C0+C1 |
