...
| DT2 | loaded deck has no flashcards and study CTA must be disabled while creation/import remain available | `FlashcardListState` contains no items, zero progress, and manual sort mode | flashcard list renders the toolbar and empty deck branch | toolbar `Import`, empty-state `No flashcards yet`, and `Add` are visible, while primary `Study this deck` has `onPressed == null` | C0+C1 |
