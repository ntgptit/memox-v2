...
| DT5 | recent deck highlights have more than three items and include due counts | dashboard state contains four deck highlights with at least one `lastStudiedAt` and the first deck has due cards | dashboard renders deck highlight section | `Pick up where you left off` appears, only the first three deck rows render, first deck metadata shows due count plus card count, the first deck study action badge shows due count, and the deck row keeps enough height for its trailing study action instead of clipping into the page background | C0+C1 |
