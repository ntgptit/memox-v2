...
| ID | Branch / condition | Given | When | Then | Coverage |
| --- | --- | --- | --- | --- | --- |
| DT1 | loaded overview has overdue reviews, due-today reviews, new cards, one active session, and library progress metrics | `DashboardOverviewState` has `overdueCount=3`, `dueTodayCount=2`, `newCardCount=7`, `activeSessionCount=1`, `folderCount=2`, `deckCount=3`, `cardCount=20`, and `masteryPercent=30` | dashboard renders the loaded data branch | greeting header replaces the old plain app-bar `Home`, `MxProgressRing`, Library progress, `30%`, `View library`, compact Today Review, New Study, Resume rows, and their labeled counts are visible | C0+C1 |
| DT2 | loaded overview has no review cards, no new cards, and no active sessions | `DashboardOverviewState` has zero counts for review, new study, and resume actions | dashboard renders the loaded data branch | Review and Resume render disabled secondary buttons, Start renders a disabled primary button, the Library progress ring remains visible, and Resume explains that no active session exists | C0+C1 |
| DT3 | dashboard study action labels must stay concise while mixed enabled states preserve CTA hierarchy and equal geometry | `DashboardOverviewState` has no review cards, new cards available, and no active sessions so Review and Resume are disabled while Start is enabled | dashboard renders the loaded data branch | action labels are `Review`, `Start`, and `Resume`, old long labels are absent, Review and Resume use secondary button surfaces, Start uses the primary button surface, and all three action containers resolve to the same compact visual width | C0+C1 |
...
| ID | Branch / condition | Given | When | Then | Coverage |
| --- | --- | --- | --- | --- | --- |
| DT1 | dashboard renders decision-first copy on compact-mobile density target | loaded dashboard state renders at logical size `412x915` with review, new-study, resume, and library progress data | dashboard lays out the first viewport | greeting helper copy is hidden, Today Review/New Study/Resume show concise `5 due`, `7 new`, and `1 active` statuses, duplicated action metadata and `30% mastery` supporting copy are absent, the structural library summary remains visible, the progress ring uses `AppLayout.dashboardChartSize(context)`, and the action list card stays within the viewport width | C0+C1 |
