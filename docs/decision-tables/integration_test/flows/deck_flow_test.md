...
| --- | --- | --- | --- | --- | --- |
| DT4 | deck detail route points to a deck id that is missing from local storage | the in-memory database is empty and the initial route is `/library/deck/e2e-missing-deck` | the deck detail provider loads the route id | `Something went wrong` and `Deck not found.` are rendered without an uncaught widget exception | C0+C1 |
