...
| DT17 | compact viewport opens a deck route whose id is missing | the app starts at `/library/deck/e2e-compact-missing` on compact viewport | the deck detail provider loads the route id | `Something went wrong` and `Deck not found.` are visible | C0+C1 |
...
| DT19 | deck detail overview renders zero-card summary | `E2E Empty Overview Deck` exists with no flashcards | the deck detail route opens | `Overview` and `0 cards · 0 due today · 0% mastery` are visible | C0+C1 |
| DT20 | deck detail overview renders never-studied metadata | `E2E Never Studied Deck` exists with no study history | the deck detail route opens | `E2E Never Studied Deck` and `Last studied: Never` are visible | C0+C1 |
