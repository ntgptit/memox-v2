...
| --- | --- | --- | --- | --- | --- |
| DT1 | database opens a pre-release schema that needs v2 study-table repair and later TTS settings creation | sqlite file contains the older study schema shape before migration and no `tts_settings` table | app database opens and migration runs to the current schema | repaired study tables exist with the expected columns and the v9 `tts_settings` table exists with audio-control columns | C0+C1 |
