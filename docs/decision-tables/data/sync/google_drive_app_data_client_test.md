...
| DT4 | Drive returns unsuccessful status with structured error reason | fake HTTP client returns HTTP 403 JSON with reason `accessNotConfigured` for media download | `downloadFile` runs | typed `GoogleDriveAppDataException` includes status code 403 and the parsed Drive reason | C0+C1 |
