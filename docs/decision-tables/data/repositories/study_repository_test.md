...
| DT16 | single-mode New Study completes without advancing to the Mix next mode | new-study session is started with only Review mode | repository batch Review answer runs with `grade=correct` | session status becomes `ready_to_finalize`, no Match queue is inserted, and `study_flow` is `new_review_only` | C0+C1 |
