...
| DT12 | Match batch submit uses a grade that is not `correct` or `incorrect` | new-study session has pending Match items | repository match batch is requested with `remembered` for one item | request fails fast with validation and no attempts are inserted | C0+C1 |
