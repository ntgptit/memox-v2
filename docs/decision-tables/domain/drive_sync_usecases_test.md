...
| DT4 | upload-local use case delegates to repository boundary | fake repository returns `uploadedLocal` | `UploadLocalDriveSnapshotUseCase.execute` runs | returned result matches repository result and upload-local is called once | C0 |
| DT5 | restore-Drive use case delegates to repository boundary | fake repository returns `restoredRemote` | `RestoreDriveSnapshotUseCase.execute` runs | returned result matches repository result and restore-Drive is called once | C0 |
