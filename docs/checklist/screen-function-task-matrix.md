...
| Current | Flashcard edit | Bury / Suspend | `BuryFlashcardUseCase`, `UnburyFlashcardUseCase`, `SuspendFlashcardUseCase`, `UnsuspendFlashcardUseCase` | `lib/domain/study/usecases/study_usecases.dart`, `lib/domain/study/ports/study_repo.dart`, `lib/data/repositories/study_repo_impl.dart`, `lib/data/datasources/local/tables/flashcard_progress_table.dart` | `docs/business/study-actions/bury-suspend.md`, `docs/contracts/usecase-contracts/study.md` §Bury/Suspend | `docs/wireframes/08-flashcard-edit.md`, `docs/wireframes/25-shared-bottom-sheets.md` §card-actions | P0-2 done: schema v10 (`buried_until`, `is_suspended`) + use cases + repo persistence/filtering. Decision rows BS1/BS2. Covered by `test/data/repositories/study_repo_next_due_test.dart` (BS1/BS2) + migration test. Trigger lives in the study-session card-actions sheet (§25), not the editor screen. |
...
Parity audit: row #13 in `docs/checklist/wireframe-code-parity-assessment.md` §1 — minor drift (doc beyond code). Swipe + scroll layers clean, but `grep onLongPress lib/presentation/features/study` = 0 handlers. Card-actions sheet now exists (`MxCardActionsSheet`, §25). §3.1 bury/suspend foundation is done; remaining P1 work is surfacing the trigger via long-press/overflow inside this mode view.
...
| Partial | Shared | Card actions (bury/suspend/edit) | `MxCardActionsSheet` using `MxActionSheetList` | `lib/presentation/shared/dialogs/mx_card_actions_sheet.dart`, `lib/presentation/features/study/screens/study_session_screen.dart`, `lib/presentation/features/study/providers/study_session_notifier.dart` | `docs/business/study-actions/bury-suspend.md` | `docs/wireframes/25-shared-bottom-sheets.md` §card-actions | P0-2: sheet built (Edit/Bury/Suspend, no History) + wired into study-session app bar with undo toast. Covered by `test/presentation/card_actions_sheet_test.dart`. Partial: surfacing the trigger inside each in-session mode view (review/match/guess/recall/fill) is follow-up. |
