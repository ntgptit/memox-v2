...
| Current | Flashcard create | Tag input | `TagValidator` gates draft tags (no comma, max 50, lowercased); shared `MxTagInput` with inline validation | `lib/domain/tag/tag_validator.dart`, `lib/presentation/shared/widgets/mx_tag_input.dart`, `lib/presentation/features/flashcards/widgets/flashcard_editor_form.dart`, `lib/presentation/features/flashcards/screens/flashcard_editor_screen.dart` | `docs/business/tags/tag-system.md`, `docs/contracts/usecase-contracts/tag.md` | `docs/wireframes/07-flashcard-create.md`, `docs/wireframes/22-settings-tag-management.md` | Create + edit share one path: validate/normalize via `TagValidator`, keep in draft, persist with the card on save (no presentation→data access). `EnsureTagUseCase` from the prompt maps to `TagValidator` + (edit-mode) `AddTagToCardUseCase`. Covered by editor tag tests in `test/presentation/flashcard_editor_screen_test.dart`. |
