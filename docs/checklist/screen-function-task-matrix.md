...
Parity audit: row #12 in `docs/checklist/wireframe-code-parity-assessment.md` §1. **All 6 Tier 1 cases now `Current`** (deck_noCards, deck_noDueCards, folder_noCards, folder_noDueCards, today_allDone, today_noContent) — typed `EmptyScopeReason` + repo scope-probe queries (`countFlashcardsInScope`, `countDueCardsInScope`, `nextDueAt`) + `EmptyScopeScreen` switch arms + 16 l10n keys. Remaining: tag scope (`tag_noCards`/`tag_noDueCards`, Tier 2, blocked on `StudyEntryType.tag`) and `allBuried`/`allSuspended` (Tier 3, blocked on §3.1 bury/suspend). Cross-cutting: §3.7 empty-scope matrix (Tier 1 resolved; Tier 2/3 still open).
