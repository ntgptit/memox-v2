...
Parity audit: row #17 in `docs/checklist/wireframe-code-parity-assessment.md` §1 — minor drift (doc beyond code), P1. UI Mark-correct + Hint button present. [Prompt 06, 2026-05-31] Strict matcher and hint reveal policy promoted to domain (`lib/domain/study/fill/fill_answer_matcher.dart`, `lib/domain/study/fill/fill_hint_policy.dart`); `FillModeSessionView` consumes them and no longer uses normalized comparison. [Prompt 07, 2026-05-31] Fill wrong feedback no longer mounts `StudyAutoSpeakEffect`; manual `StudySpeakButton` remains post-feedback and `test/presentation/fill_mode_session_view_test.dart` covers typing hidden, no auto-play with `autoPlay=true`, and manual speak. Hint taint (`perfect → recovered` demotion when hint used) at SRS finalization is NOT yet implemented — `AttemptGrade` is binary and downgrading without faking an `incorrect` attempt needs a schema-affecting follow-up. Cross-cutting: §3.10 strict matcher / hint taint (partially resolved); §3.13 items 6–7 (now ✅).
...
| Current | Session/Fill | Hint reveal | `FillHintPolicy` (domain) + per-card reveal state in `FillModeSessionView` | `lib/domain/study/fill/fill_hint_policy.dart`, `lib/presentation/features/study/widgets/study_session/fill/**` | `docs/wireframes/17-study-session-fill.md` §Components (Hint button) + §Matching rules | `docs/wireframes/17-study-session-fill.md` | Covered by `test/domain/study/fill_hint_policy_test.dart` + `test/presentation/fill_mode_session_view_test.dart`. Hint-tainted → SRS downgrade still Blocked (see §3.10). |
