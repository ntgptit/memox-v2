...
Parity audit: row #12 in `docs/checklist/wireframe-code-parity-assessment.md` §1. **All 6 Tier 1 cases now `Current`** (deck_noCards, deck_noDueCards, folder_noCards, folder_noDueCards, today_allDone, today_noContent) — typed `EmptyScopeReason` + repo scope-probe queries (`countFlashcardsInScope`, `countDueCardsInScope`, `nextDueAt`) + `EmptyScopeScreen` switch arms + 16 l10n keys. **Tier 3 (`allBuried`/`allSuspended`) now `Current`** (P0-2, §3.1 resolved). Remaining: tag scope (`tag_noCards`/`tag_noDueCards`, Tier 2, blocked on `StudyEntryType.tag`). Cross-cutting: §3.7 empty-scope matrix (Tier 1 + Tier 3 resolved; Tier 2 open).

| Status | Screen | Function | Function detail | Files to modify | Doc refs | Wireframe | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Current | Entry gate | Resolve resume vs start | `ResumeStudySessionUseCase.findCandidate` + Resume/Start-over choice (`MxDialogResumeOrStartOver`) → Resume reuses the session; Start over → discard confirm (`MxConfirmationDialog`) → `RestartStudySessionUseCase` (flow-preserving via optional `modes`); resume offered only when candidate `study_flow` == intended flow; Cancel pops back without creating a session | `lib/presentation/features/study/screens/study_entry_screen.dart`, `lib/presentation/shared/dialogs/mx_dialog_resume_or_start_over.dart`, `lib/domain/study/usecases/study_usecases.dart` | `docs/business/study/study-flow.md`, `docs/business/resume/resume-session.md`, `docs/contracts/usecase-contracts/study.md`, `docs/wireframes/24-shared-dialogs.md` §resume-or-start-over | `docs/wireframes/12-study-entry-gate.md` | Prompt 05 (2026-05-30): replaced silent auto-resume with the spec'd Resume/Start-over dialog. Covered by `test/presentation/study_entry_screen_test.dart` (shows-choice / Resume / Start over / Cancel / mode-mismatch) + `test/integration/study_progress_data_flow_test.dart` (resume + multi-mode continue). |
