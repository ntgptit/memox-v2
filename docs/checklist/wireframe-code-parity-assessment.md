...
| 19 | `19-settings-account.md` | 🟡 Partial | Sign in Google → link → manual Drive upload/restore. | [Prompt 21, 2026-05-31]: route/sub-screen safety verified only. Account sign-in and manual Drive upload/restore are implemented through account/sync use cases and settings viewmodels. **Pending/Target:** full restore protection in the wireframe/business doc (pre-restore local snapshot notice/path, Upload local first branch, second destructive confirmation) was not promoted by Prompt 21. | Dedicated Account/Drive parity prompt should verify or implement restore-protection target behavior. | Sync layer and UI owner are concrete. | Restore warning/safety parity still Partial. |
