...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 01 | `01-dashboard.md` | 🟡 D>C (engagement-only) | Resume card (multi-session), static streak stat placeholder, due cards summary, mastery, deck highlights. | [Prompt 04, 2026-05-30]: Resume card (Continue/Discard + "+N more"), **multi-resume** paused-sessions sheet, discard dialog, "Start new learning" scope picker, và recent decks đã implement + tested (`test/presentation/dashboard_screen_test.dart`); `DashboardOverviewState.resumeSessions` carries the list. [Prompt 32, 2026-06-02]: static `Streak` / `0 days` stat is kept as a visual/stat placeholder only and covered by a negative engagement-action test. **Còn lại chỉ là engagement**: `MxStreakCard` (`mx_streak_card.dart:14`) vẫn không có call-site; state không có field streak/dailyGoal — streak chip/history + daily-goal ring/settings/reminders stay `Target`/Future (blocked on §3.2). | **P2 (engagement)**: Implement `EngagementUseCase` (streak từ distinct `study_sessions.started_at`); mount `MxStreakCard` vào `dashboard_content.dart` khi product decision được chốt. | Route + viewmodel + widgets + resume/scope/recent đều ổn. | Chỉ full streak/daily-goal/reminder engagement còn trống; study-entry/resume parity đã đạt. |
| 02 | `02-library.md` | 🟡 Partial / V1 folders-only | Library: folders-only V1 surface with inline scope-local search and Create folder. Root-level decks are Rejected / Out of Scope. | [Verified 2026-05-28]: Library overview dùng `MxSearchField` inline trong app bar (`library_app_bar.dart:60`); KHÔNG dùng full toolbar — Rev 2 nhầm với folder-detail. `libraryToolbarStateProvider.query` scope-local đúng spec. [Prompt 14, 2026-05-31]: Recursive folder count verified. [Prompt 18, 2026-05-31]: **folders-only reality** — `LibraryOverviewReadModel` has no root-deck channel. [Prompt 43A, 2026-06-03]: §Layout deck rows + deck-row nav are Rejected / Out of Scope, not Future implementation backlog. Fixed search-no-results bug (mirrors Prompt 16B). [Prompt 18B, 2026-05-31] corrected the classification to use `LibraryOverviewState.totalFolderCount`: true empty = `totalFolderCount == 0` (any term → "Create folder"); search no-results = `folders.isEmpty && searchTerm active && totalFolderCount > 0` → `LibrarySearchNoResultsSection` ("Clear" CTA). Widget tests DT3/DT4/DT4b/DT5. No `/library/search` route exists (search inline-only, never navigates). FAB creates folder only; sort exists at repo/use-case layer but has no UI control. Fixed stale wireframe code-path refs (`features/library/*` → `features/folders/*`). [Prompt 49, 2026-06-04]: 6-state visual parity (skeleton/error/kebab). [Prompt 49B, 2026-06-04]: **loaded-state visual drift fixed** — header sliders/`Icons.tune_rounded` (disabled, visual-only target) replaces search toggle; **always-visible inline search**; static `All` chip removed; `{n} FOLDERS` overline header (count only); non-interactive **due summary card** when `dueToday > 0`; FAB → labelled **`New folder` pill** (no New deck / Import). New l10n: `libraryFiltersTooltip`/`librarySearchHint`/`libraryNewFolderLabel`/`libraryFolderCountLabel`/`libraryDueSummaryTitle`. No schema/SRS/repository/use-case change. Tests in `library_overview_screen_test.dart` (49B group). | Recursive-count + empty/no-results P2 resolved; loaded chrome fixed in 49B; Library stays Partial for sort UI and a real filter sheet. | Search + create-folder + empty/no-results clean and tested; loaded layout now matches mock; root-deck direction locked out. | Root decks rejected; sort control + filter sheet behind sliders icon + due-card subtitle remain target-only. |
| 03 | `03-progress.md` | 🟡 Partial / V1 honest | V1 Progress Overview: learning summary metrics + active session recovery. Future analytics target: cards/day chart, accuracy chart, box distribution, streak/daily-goal, suspended/buried global links, Flashcard History. | [Prompt 20, 2026-05-31]: `/progress` route opens `ProgressScreen` safely inside shell (`progress_routes.dart`, `app_router.dart`, `route_names.dart`); `progressOverviewProvider` combines `WatchLibraryOverviewUseCase` due/new/mastery/card-count data with `ResumeStudySessionUseCase.listActiveSessions`; UI renders due now/new cards/mastery/active sessions plus active/ready/failed session recovery cards. Loading uses `MxRetainedAsyncState` shared loading, failure maps to `MxErrorState` with Try again and no raw exception by default, empty active-session state shows Library CTA and no fake analytics. **Future/not exposed:** range chips, cards-studied chart, accuracy chart/review accuracy, box distribution, streak/daily goal, suspended/buried links, completed recent-history list, Flashcard History, Global Search. | Prompt 20 closes V1 route/action/state/metric/session parity honestly; keep analytics widgets as Future until dedicated progress repository/use-case is approved. | Current V1 surface is safe and tested; docs now name real owners. | Whole analytics wireframe is not Current; no dedicated progress repository/usecase exists yet. |
| 04 | `04-settings-hub.md` | 🟡 Partial / V1 route-action Current | Hub Account/Learning/Audio-Speech/Tags/About. | [Prompt 21, 2026-05-31]: `/settings` renders `SettingsScreen` inside shell; account/learning/audio/tags rows push named routes; account/learning/audio/tags sub-routes hide shell; pushed account route back returns to hub. Hub owns navigation only, not account/Drive/TTS/study/tag mutation. Account error row remains navigable; row semantics include subtitle. Appearance/Language disabled with Soon badge. **Partial honesty:** Learning/audio/tags subtitles are V1 summaries, not full live aggregates; About current is Flutter `AboutDialog`, while bottom-sheet remains Target/release polish. | Keep hub Current for route/action safety; keep subtitle/About as Partial/Target until a dedicated Settings polish task. | Real route/action safety now tested; scope leaks avoided. | Full dynamic subtitle contract and About bottom-sheet not Current. |
| 05 | `05-folder-detail.md` | 🟡 Partial / V1 browsing Current | Breadcrumb, children list, sort, search inline, quick actions, empty state, lock-mode rejection. | [Verified 2026-05-28]: §Components đã document `MxSearchSortToolbar<ContentSortMode>` (verified at `folder_detail_screen.dart:178`). [Prompt 14, 2026-05-31]: lock-mode fallback UX verified and fixed: `FailureCodes.folderContainsDecks` / `folderContainsSubfolders` map to localized snackbar copy in Folder Detail; widget tests cover stale create-subfolder/create-deck paths and assert no generic error appears. [Prompt 19, 2026-05-31]: verified Current — route/invalid-id safety (`NotFoundException`→`MxErrorState`+Retry), content-mode rendering (never mixed), inline scope-local search (no Global Search nav, no order mutation), sort (`ContentSortMode` at repo layer + `MxSortMenuChip`), create subfolder/deck + typed lock snackbar, row actions (Edit/Move/Delete/Import/Duplicate/Export), manual reorder. **Fixed true-empty vs search no-results** via `FolderDetailReadModel.hasUnfilteredChildren` → `FolderDetailState` (mirrors Prompt 18B): a genuinely empty locked folder stays true-empty for any search term; no-results only when children exist but filtered out. **Honesty correction:** Folder Detail exposes NO Study folder / Today / Resume CTA — reclassified matrix "Start study (folder scope)" Current→Future; those remain study-entry-gate (§12) / resume work. Fixed stale wireframe code paths (`features/library/*`→`features/folders/*`). [Prompt 45, 2026-06-03]: **Study/Today/Resume banners promoted Future→Current** via `FolderStudyEntrySection` + `GetFolderStudyEntryUseCase` (reuses `StudyRepo.countFlashcardsInScope`/`countDueCardsInScope`/`findResumeCandidate`; no schema). Study folder → gate `entry_type=folder`; Today (shown iff due>0) → gate `entry_type=folder&study_type=srs_review` (new additive `study_type` query param parsed in `study_entry_screen.dart`); Resume opens existing session directly. No gate bypass. Tests: `folder_study_entry_section_test.dart`, `folder_study_entry_usecase_test.dart`, study-entry `study_type` cases in `study_entry_screen_test.dart`. | Lock-mode + empty/no-results + error/route safety resolved; study-entry banners now Current. | Shared toolbar reused; lock-mode typed + localized; study banners route through the gate. [Prompt 47, 2026-06-03]: Resume **Discard** promoted Future→Current via the shared `confirmAndDiscardResumeSession` flow (`CancelStudySessionUseCase`; never creates a session); P47 group in `folder_study_entry_section_test.dart`. [Prompt 50, 2026-06-04]: **Decks-mode hero card + mastery ring promoted Future→Current** (`FolderHeroCard`): data-backed mastery ring (avg deck `masteryPercent`) + `Folder mastery` overline + `{deckCount} decks · {cardCount} cards` + `{dueCount} due` + folder-scoped Start study CTA (same Study Entry Gate contract + action keys; never starts a session; Resume banner rides above via `FolderStudyEntrySection.showStudyCard`). Compact `{n} DECKS`/`{n} SUBFOLDERS` section overline (`FolderSectionTitle`) above the unchanged scope-local toolbar. Tests: P50 hero cases in `folder_study_entry_section_test.dart`, `button_label_contract_test.dart`. | **"{n} new" subtitle remains Future** (no read model — never rendered as a placeholder); decks-mode FAB stays `New deck` (mock `New card` needs a specific deckId; auto-pick unsafe); move=NotStarted (no UI), broader cascade-copy parity separate. |
| 06 | `06-flashcard-list.md` | ✅ = (verified V1 scope) | Danh sách: route safety, deck summary, study/import ownership, inline search, sort/reorder, row/bulk delete/move/export, empty/loading/error/no-results states. | [Verified 2026-05-28]: §Components đã expand thành 13-row bảng mapped 1-1 với `flashcard_*_section.dart` files theo render order screen (lines 130-436). [Prompt 16/16B, 2026-05-31]: §Rules / §States / §Forbidden re-verified. Current: invalid `deckId` safe error + retry, loading/error/empty-deck states, distinct no-results-on-search (`FlashcardNoResultsSection`, corrected to require `totalCount > 0`), scope-local search, sort + manual reorder, row/bulk Delete-Move-Export ownership, study modes via Study Entry gate, and Import route ownership. Future/not exposed: status/tag filter chips, state badges + `CardStateComputer`, bulk suspend/unsuspend/reset/tag, long-press-to-select. [Prompt 46, 2026-06-03]: **Deck Study/Today/Resume banners promoted Future→Current** via `FlashcardStudyEntrySection` + `GetDeckStudyEntryUseCase` (reuses `StudyRepo.countFlashcardsInScope`/`countDueCardsInScope`/`findResumeCandidate`; no schema), mirroring Folder Detail (Prompt 45). Study deck → gate `entry_type=deck`; Today (shown iff due>0) → gate `entry_type=deck&study_type=srs_review` (deck-scoped due review, NOT global `entry_type=today`, NOT `?type=`); Resume opens existing session directly. No gate bypass; never starts a session itself. Tests: `flashcard_list_screen_test.dart` (P46 group), `deck_study_entry_usecase_test.dart`, deck `study_type` cases in `study_entry_screen_test.dart`. | Prompt 16/16B closed the stale P3 follow-up; Prompt 46 promoted the deck study-entry banners. Keep remaining Future items unpromoted. | Mapping doc ↔ widget rõ ràng; states/rules/forbidden ownership verified; deck study banners route through the gate. [Prompt 47, 2026-06-03]: Resume **Discard** promoted Future→Current via the shared `confirmAndDiscardResumeSession` flow (`CancelStudySessionUseCase`; never creates a session); tests in `flashcard_list_resume_discard_test.dart` (P47 group). | Whole target wireframe still broader than V1 because status/tag filters, badges, and bulk suspend/reset/tag remain Future. |
| 07 | `07-flashcard-create.md` | ✅ = (shared-editor contract scope) | Form tạo flashcard 1 thẻ; route riêng `flashcardCreate`. | [Prompt 15, 2026-05-31]: `flashcard_editor_screen.dart` shared cho cả create và edit (`flashcardCreate` và `flashcardEdit` cùng map sang screen này). Wireframes 07/08 now remain route-specific but both carry a shared Flashcard Editor contract table; create mode is `deckId` with no `flashcardId`. [Prompt 15B, 2026-05-31]: dirty close/back now asks discard confirmation for typed content/tags/optional fields/status/destination changes; normal Save after destination retarget returns to the selected deck list; Save and add another keeps selected deck and resets to a clean draft. | P2 doc-organization issue resolved by cross-ref/shared-contract notes instead of deleting route-specific files. | Một code path, ít drift logic; docs now name the shared owner. | Full visual parity beyond shared-editor contract remains scoped to later screen audits. |
| 08 | `08-flashcard-edit.md` | ✅ = (V1 owner/scope contract) | Shared edit form + explicit learned-content progress policy; no live History action. | [Prompt 15, 2026-05-31]: Edit route uses the shared editor with `flashcardId`. `View history` remains Future Proposal and is not exposed in V1. Move/delete/export live on flashcard list row/bulk actions; Bury/Suspend live in the study-session card-actions sheet (#25), not this editor. The editor's only progress-affecting path is the explicit Keep/Reset policy dialog for learned front/back edits. [Prompt 15B, 2026-05-31]: dirty close/back is guarded by discard confirmation; existing optional note/tags do not dirty the form until changed; optional field and tag edits do dirty the form. | Keep History hidden until the Future card-history migration/scope promotion. | Bury/Suspend functional via card-actions sheet; delete/move/export owner documented honestly. | Standalone History reset remains Future/migration-required. |
...
| 10 | `10-deck-import.md` | ✅ = (V1 bulk-add owner/scope) | Import CSV/Excel/structured-text vào deck. | [Verified 2026-05-28]: code paths `flashcard_import_support.dart` (csv + structuredText) + `flashcard_excel_import_parser.dart` (DIY xlsx, single-sheet). Format enum at `value_objects/content_actions.dart:45`. [Prompt 17, 2026-05-31]: Decision-table rows cross-verified — route/ownership (router DT2), formats + separators + Excel (import_support DT1–DT8), validation/canCommit, commit + sort_order + progress init + no-partial-insert (content_repo DT3/DT7), in-file/in-deck dedupe (DT8/DT9), unknown-deck safety (new DT10/DT11). Wireframe gained "V1 verification status": shipped UI is inline bulk-add (mock 05d), NOT the 3-step/result mock. | ~~P3 cross-verify~~ → **DONE (Prompt 17)**. | V1 bulk-add: 3 formats, skip-exact dup, inline preview. | 3-step flow, result screen, per-row skipped list, extra dup policies, discard dialog = Future. |
...
| 19 | `19-settings-account.md` | 🟡 Partial / V1 account-sync Current | Sign in Google → link → manual Drive upload/restore. | [Prompt 22, 2026-05-31]: `/settings/account` route/sub-screen safety verified; Account detail owns sign-in/sign-out/disconnect, safe signed-out/unconfigured/unsupported/error states, and manual Drive upload/restore. Sign-in failure no longer renders auth technical detail. Drive sync status covers signed-out, unconfigured, reconnect, no-remote, ready, synced, unsupported-schema, and failure; `synced` now disables manual sync action. Settings Hub remains navigation-only and exposes none of these mutations. **Pending/Target:** full restore protection in the wireframe/business doc (pre-restore local snapshot notice/path, Upload local first branch, second destructive confirmation), token-expired banner, and account removal/switch-account strong-confirm flow were not promoted by Prompt 22. | Full restore-protection and account-removal require dedicated code + tests + docs; keep Partial until then. | Sync layer and UI owner are concrete; current manual V1 behavior is tested. | Restore warning/safety target and account-removal target still Partial/Future. |
| 20 | `20-settings-learning.md` | 🟡 Partial / V1 learning-settings Current | Current V1: new-study/review defaults, shared study toggles, read-only runtime interval table, Manage tags route entry. Target: daily goal, streak, reminder. | [Prompt 23, 2026-05-31]: Learning Settings now renders the interval table from `SrsIntervalPolicy`, the same runtime source used by SRS finalization via `_intervalForBox`; Manage tags pushes `/settings/learning/tags` while tag CRUD remains owned by the tag management screen. Code edits study defaults through `StudyDefaultsSettings`; daily-goal/streak/reminder controls remain Future/Target and tied to engagement scope. | Keep engagement settings out of Learning Settings until dedicated engagement/settings use cases exist. | Study-defaults, interval display, and tags route ownership are clear and tested. | Original wireframe layout still contains target engagement controls. Canonical interval ladder remains a P2 product/docs decision. |
| 21 | `21-settings-audio-speech.md` | ✅ V1 global TTS Current / 🟡 Target per-language Future | Current V1: global/front-language TTS settings. Target: independent per-language tabs/settings. | [Prompt 24, 2026-06-01]: verified `/settings/audio-speech` route/action safety, direct-route shell hiding, pushed back-to-hub behavior, and no Account/Learning/Search/History/Progress scope leak. Code uses one `TtsSettings` (`autoPlay`, `frontLanguage`, `rate`, `pitch`, `volume`, `frontVoiceName`) and a detail sheet; voice options are collapsed/lazy, selected voice persists, language change clears stored voice, preview uses `TtsController`/`SpeakFlashcardUseCase`/`TtsService`, and preview failure renders generic localized feedback without raw platform exceptions. Docs corrected current storage to Drift table `tts_settings` and default volume to `1.0`. | Keep the target per-language design as Future until a dedicated implementation/migration prompt adds independent settings with tests. | V1 route/action, persistence, preview, voice options, and safe failure behavior are covered by focused settings/router/TTS tests. | Independent Korean/English tabs/settings, reset current tab, play-after-grading expansion, unsupported-language explainer, eager unsupported-engine remediation, and per-language storage are not Current. |
| 22 | `22-settings-tag-management.md` | ✅ = (V1 scope) | Tag list, rename, merge, delete; affect flashcards cascade. | [Verified 2026-05-30]: Tag domain layer added — `TagRepository` + `TagRepositoryImpl` + `FlashcardTagDao` + `TagValidator` + use cases (`WatchAllTagsWithCount`/`AddTagToCard`/`RemoveTagFromCard`/`RenameTag`/`MergeTag`/`DeleteTag`). Screen rewritten with rename/merge/delete via shared dialogs; UseCase → Repository → DAO flow (no data access from presentation). Tags lowercased (schema v11). [Prompt 25, 2026-06-01]: trust sweep re-verified `/settings/learning/tags` route/shell ownership, list/loading/empty/error/search/sort behavior, rename/merge/delete safety, usage-count distinct-card semantics, and safe localized failure feedback. Docs now explicitly keep tag-scoped study Blocked on `StudyEntryType.tag` and global tag-filtered card list Future / Global Search ownership. | — (resolved) | `mx_tag_input` shared widget (now with inline validation). | "Study cards with this tag" Blocked (`StudyEntryType.tag`); global "View cards" Future (global search) — not exposed in V1. |
| 23 | `23-onboarding.md` | 🟡 V1 thin owner-split / Future full | V1 zero-content guidance reuses existing create/import/restore owners; full onboarding flow is future. | [Verified 2026-05-28]: `grep -i "onboarding" lib/` empty and initial route is Library. [Scope 2026-05-29]: this is acceptable for V1; implement stronger empty-state CTAs, not standalone onboarding. [Prompt 26, 2026-06-01]: re-verified no `/onboarding` route, no onboarding feature folder, no first-launch gate, no wizard. Added focused CTA tests for Library true-empty Create folder, Folder Detail empty New subfolder/New deck/Clear search, Flashcard List empty Add, and route guard default entry. Account Settings remains the only Current restore owner. | No standalone onboarding work remains in V1. Dashboard zero-content onboarding stays Target/Future unless a dedicated Dashboard task implements it. | Avoids M-size onboarding scope and keeps restore ownership in Account Settings. | Full welcome/restore prompt remains future; no standalone restore wizard. |
| 24 | `24-shared-dialogs.md` | 🟡 V1 primitives Current / Target catalog partial | Catalog 8+ dialogs. | [Verified 2026-05-28]: Code có 3 typed widgets (`mx_dialog`, `mx_confirmation_dialog`, `mx_name_dialog`). [Prompt 05, 2026-05-30]: **§resume-or-start-over implemented** as `MxDialogResumeOrStartOver` (`mx_dialog_resume_or_start_over.dart`, typed `MxResumeChoice`); §discard-session composed via `MxConfirmationDialog` (danger). [Prompt 27, 2026-06-01]: re-verified Current primitives (`MxDialog`, `MxConfirmationDialog`, `MxNameDialog`, `MxDialogResumeOrStartOver`) and composed destructive/name/resume usages. Fixed destructive confirmation primitive so danger dialogs are not barrier-dismissible and added focused coverage. Strong-confirm typed input, account removal, full restore-warning two-tier flow, onboarding dialogs, and dialog gallery remain Target/Future. | No dedicated V1 gallery needed. Keep catalog Target items unpromoted until implemented with tests. | Current primitives and key composed usages are tested. | Specific catalog widgets remain partial; many Current flows compose the primitive instead of using one file per dialog. |
| 25 | `25-shared-bottom-sheets.md` | 🟡 Partial / V1 honest | Shared modal host, action list, destination picker, card-actions, paused sessions, scope picker, sort/engagement target sheets. | [Prompt 28, 2026-06-01]: `MxBottomSheet` now renders the required visible/tappable drag handle and keeps keyboard-inset handling; `MxActionSheetList`, `MxDestinationPickerSheet`, and `MxCardActionsSheet` are Current primitives with focused tests. `MxDestinationPickerSheet` remains data/callback-driven and covers disabled destinations, empty state, initial query, subtitle/searchTerms matching. Study-session `MxCardActionsSheet` exposes only Edit / Bury / Suspend (no History) and bury/suspend drop the active card (no requeue). Dashboard paused sessions are Current as a Dashboard feature-local sheet. Dashboard Start New Learning and Study Result Study More share the two-step Today/Deck/Folder scope picker; Tag scope remains Blocked/Future. Dedicated `SortOptionsSheet` is not implemented; current sort UI is chip/menu based. `DailyGoalSheet`, `StreakHistorySheet`, reminder/notification sheets, engagement sheets, Global Search, and Flashcard History remain Target/Future/Blocked. | Keep catalog Partial; do not promote target/future sheets without code + tests + docs. Future follow-up: dedicated sort sheet only if product wants bottom-sheet sort UX; active-session undo reinsert remains follow-up. | Current V1 primitives/usages are tested and docs no longer imply unimplemented engagement/sort/tag/history sheets are Current. | Catalog still contains many target sheets; paused sessions intentionally feature-local rather than a fully generic shared sheet. |
...
| `settings` (+ 4 sub) | `/settings/*` | Settings hub + 4 sub-screens | ✅ (Prompt 21 route/action safety; subtitle/About polish Partial) |
...
| **Future / intentionally absent** | — | `/flashcard/:id/history` (wireframe 09) | ⚪ |
| **Future / intentionally absent** | — | `/library/search` (wireframe 11) | ⚪ |
| **Future / intentionally absent** | — | `/onboarding` (wireframe 23) | ⚪ |

`RouteDefaults.initialLocation = RoutePaths.library` — Prompt 26 verified this is the current V1 app entry. Changing default entry to `/home` remains a separate navigation task, not onboarding scope.
...
Standalone onboarding remains absent by design. Prompt 26 re-verified V1 has no onboarding route/feature folder, no first-launch gate, and no wizard. Current V1 zero-content guidance is owner-split: Library owns local Create folder, Folder Detail owns mode-appropriate create CTAs, Flashcard List owns Add + deck import entry points, and Account Settings owns sign-in/manual Drive restore. Full welcome screen, onboarding feature folder, restore prompt branch, and Dashboard zero-content onboarding are Future/Target until separately implemented with code + tests + docs.
...
```
| `lib/domain/study/srs_interval_policy.dart` | `docs/business/srs/srs-review.md` (interval table) |
...
- `lib/domain/study/srs_interval_policy.dart` (`SrsIntervalPolicy`) and `lib/data/repositories/study_repo_impl_mapping_helpers.dart` (`_intervalForBox`) → `docs/business/srs/srs-review.md` interval table.
...
| --- | --- | --- | --- |
| 1 | `lib/domain/srs/box_intervals.dart` | **Resolved:** runtime interval source is `SrsIntervalPolicy` in `lib/domain/study/srs_interval_policy.dart`; `_intervalForBox` delegates to it for repository finalization | No runtime blocker |
...
**Source (current):** `SrsIntervalPolicy` in `lib/domain/study/srs_interval_policy.dart`, reached by `_intervalForBox` in `lib/data/repositories/study_repo_impl_mapping_helpers.dart`; canonical ladder still P2 product/docs decision.
```
...
| 8 | ✅ Resolved (Prompt 29, 2026-06-01). Mock variant `28a-28i` Onboarding remains a visual reference only; mapping now says the full onboarding mock set is Future Proposal and V1 implements only zero-content empty-state CTAs. | — | Done. |
...
| **P1-3** | ✅ DONE (2026-06-01, Prompt 26) Thin zero-content onboarding honesty sweep + CTA coverage | §3.6, #23 | S |
...
| **P2-2** | ✅ DONE (2026-05-31, Prompt 15) Flashcard create/edit shared-editor doc unification | #07, #08 | XS |
...
| **P2-5** | ✅ DONE (2026-06-01, Prompt 24) TTS settings V1 parity: route/action safety, global/front-language settings, voice picker collapse/lazy-load/selection, preview service path + safe failure feedback, and docs honesty verified. Target per-language tabs/settings and expanded unsupported-engine remediation remain Future. | #21 | S |
...
| **P3-5** | ✅ DONE (2026-05-31, Prompt 17) Wireframe 10 decision-table import rows cross-verified. Verified Current: route/ownership safety (router DT2 + new content_repo DT10/DT11 unknown-deck), formats CSV/structuredText/Excel + separators (import_support DT1–DT8), validation + `canCommit` gating, commit single-transaction + sort_order-after-existing + default SRS progress + no-partial-insert (content_repo DT3/DT7), in-file vs in-deck skip-exact dedupe (DT8/DT9). Documented Future (not exposed): 3-step flow, standalone result screen, per-row skipped-duplicate list with In file/In deck badges, merge/overwrite dup policies, discard dialog, chunked-insert >500 / isolate-parse >1000, AI/OCR/cloud import. Wireframe 10 gained "V1 verification status". | #10 | XS |
