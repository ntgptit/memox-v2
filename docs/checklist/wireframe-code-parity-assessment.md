...
| 25 | `25-shared-bottom-sheets.md` | 🟠 D>C | Card-actions, undo toast, destination picker, study-mode picker. | [Verified 2026-05-29]: base widgets + **`MxCardActionsSheet`** (Edit/Bury/Suspend, no History) implemented with **undo toast** (P0-2, §3.1 resolved). | **P1**: surface card-actions inside in-session mode views; scope picker / paused sessions still TBD. | Card-actions + undo done. | Sheet ecosystem ~60% complete. |
...
### §3.1 Bury / Suspend — **Foundation RESOLVED 2026-05-29 (P0-2)**
...
| Presentation | 🟡 | `MxCardActionsSheet` (Edit/Bury/Suspend, no History) wired into study-session app bar with undo toast. Remaining: surface the trigger inside each in-session mode view + flashcard-list badges/filters/bulk (follow-up). |

**Resolved**: study-batch + due-count filtering (excludes suspended + currently-buried; expired bury re-enters), empty-scope `studyEmpty_allBuried`/`studyEmpty_allSuspended`, card-actions sheet.

**Remaining follow-up (not blockers)**: mode-view long-press/overflow trigger across wireframes 13-17, flashcard-list state badges + status filter chips (06), bulk suspend/unsuspend, bury defaults in settings (20).
