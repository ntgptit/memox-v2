...
| 21 | `21-settings-audio-speech.md` | 🟡 Partial / V1 global TTS Current | Current V1: global/front-language TTS settings. Target: independent per-language tabs/settings. | [Prompt 21, 2026-05-31]: docs clarified current screen reality. Code uses one `TtsSettings` (`frontLanguage`, `rate`, `pitch`, `volume`, `frontVoiceName`) and a detail sheet; per-language tabs, reset, and unsupported-language explainer remain Future/Target. | Dedicated TTS parity task should decide whether to implement per-language target or keep global settings. | Persistence + preview path exist. | Per-language independence is not Current. |
