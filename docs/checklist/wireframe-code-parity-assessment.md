...
| 17 | `17-study-session-fill.md` | 🟡 D>C | Type front, strict char match, Mark correct override, Hint button taint to `recovered`. | [Verified 2026-05-28]: Code paths đã refresh. **Strict matcher inline trong presentation** (`fill_actions.dart` / viewmodel) — không có file domain riêng. **Hint taint** business logic chưa verify thực thi đúng (demote `perfect → recovered` khi hint dùng). [Prompt 06, 2026-05-31]: **Strict matcher promoted to domain** (`lib/domain/study/fill/fill_answer_matcher.dart`) — `FillModeSessionView` no longer uses `StringUtils.equalsNormalized`; matching is trim + strict equality (no case folding, no diacritic stripping, no whitespace collapsing). **Hint reveal policy promoted to domain** (`lib/domain/study/fill/fill_hint_policy.dart`) — `floor(len/2)` cap, per-card reveal count tracked in view state; Hint button reveals one char per tap, taps disabled at cap, Try again clears input but retains reveal count for the same card, new card resets it. **Hint-taint grading still NOT downgraded** at SRS finalization: `AttemptGrade` is binary (`correct`/`incorrect`) and the only signal to demote `perfect → recovered` in `_reviewOutcome` is an `incorrect` attempt; representing hint-tainted-but-correct without faking an incorrect attempt requires either a ternary `AttemptGrade` (schema-affecting in `study_attempts.result` codec) or a parallel hint-used flag on the attempt row. Out of Prompt 06 scope. | **P1**: extend grading channel to demote hint-tainted exact match → `recovered` without persisting a fake `incorrect` attempt (needs `AttemptGrade.recovered` or `hint_used` column). | Strict matcher + hint reveal now Clean-Architecture compliant; covered by `test/domain/study/fill_answer_matcher_test.dart`, `test/domain/study/fill_hint_policy_test.dart`, `test/presentation/fill_mode_session_view_test.dart`. | Hint-taint→SRS downgrade still missing; tracked under §3.10. |
...
**Remaining P1**: extend the grading channel to demote hint-tainted exact match → `recovered` at finalize time.
