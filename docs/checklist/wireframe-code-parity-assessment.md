...
| 02 | `02-library.md` | 🟡 Partial / V1 folders-only | Library: folders-only V1 surface with inline scope-local search and Create folder. Root-level decks remain Future/Target. | [Verified 2026-05-28]: Library overview dùng `MxSearchField` inline trong app bar (`library_app_bar.dart:60`); KHÔNG dùng full toolbar — Rev 2 nhầm với folder-detail. `libraryToolbarStateProvider.query` scope-local đúng spec. [Prompt 14, 2026-05-31]: Recursive folder count verified. [Prompt 18, 2026-05-31]: **folders-only reality** — `LibraryOverviewReadModel` has no root-deck channel, so §Layout deck rows + deck-row nav are Future (recorded in wireframe 02 "V1 verification status"). Fixed search-no-results bug (mirrors Prompt 16B). [Prompt 18B, 2026-05-31] corrected the classification to use `LibraryOverviewState.totalFolderCount`: true empty = `totalFolderCount == 0` (any term → "Create folder"); search no-results = `folders.isEmpty && searchTerm active && totalFolderCount > 0` → `LibrarySearchNoResultsSection` ("Clear" CTA). Widget tests DT3/DT4/DT4b/DT5. No `/library/search` route exists (search inline-only, never navigates). FAB creates folder only; sort exists at repo/use-case layer but has no UI control. Fixed stale wireframe code-path refs (`features/library/*` → `features/folders/*`). | Recursive-count + empty/no-results P2 resolved; Library stays Partial for root-deck rendering, sort UI, FAB action-sheet, filter chips. | Search + create-folder + empty/no-results clean and tested; honest Future list. | Root decks not rendered; sort/FAB/chips remain target-only. |
