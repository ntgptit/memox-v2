...
| 12 | `12-study-entry-gate.md` | 🟡 D>C (Tier 1 done) | Empty scope matrix 10 cases với l10n keys. | [Verified 2026-05-29]: **6 Tier 1 cases implemented**. `StartStudySessionUseCase._rejectEmptyScope` (`study_usecases.dart`) branches per entry/study type, throwing typed `EmptyScopeException(reason, nextDueAt)` for deck_noCards / deck_noDueCards / folder_noCards / folder_noDueCards / today_allDone / today_noContent. Repo probe queries `countFlashcardsInScope` / `countDueCardsInScope` / `nextDueAt` in `study_repo_impl.dart`. `EmptyScopeScreen` renders all 6 arms with CTAs; 16 l10n keys added. Tier 3 (`allBuried`/`allSuspended`) implemented 2026-05-29 (P0-2). [Prompt 05, 2026-05-30]: **resume-vs-start now matches the spec** — the silent auto-resume was replaced by the `MxDialogResumeOrStartOver` choice (Resume / Start over → discard confirm → flow-preserving `RestartStudySessionUseCase`); resume is offered only when the candidate's `study_flow` matches the entry's intended flow, and Cancel pops back without creating a session (`study_entry_screen.dart`). Remaining: Tier 2 (`tag_noCards`/`tag_noDueCards`) needs `StudyEntryType.tag`. | **P0 Tier 1 + Tier 3 + resume dialog resolved.** Remaining: Tier 2 (tag). | Typed failure + dedicated empty states + tests (decision rows S4/S4b–S4g/S4j). | 2 of 10 cases still blocked (tag scope). |
