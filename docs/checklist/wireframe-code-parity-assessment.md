...
| 15 | `15-study-session-guess.md` | ✅ = | 5 options (A-E), up to 4 valid decoys, countdown 0.8s correct / 1.5s wrong. | [Prompt 10, 2026-05-31]: Option generation moved to domain `lib/domain/study/guess/guess_option_builder.dart` (`GuessOptionBuilder.build`, `kGuessDecoyLimit = 4`, deterministic seeded shuffle, dedup by id + normalized back, blank back filter, correct exactly once). Presentation (`guess_mode_session_view.dart`) and `studyGuessAnswerOptions` delegate to the builder; no `Random`/`shuffle()` remains in the view. Countdown constants verified: `AppDurations.guessCorrectAdvanceDelay = 800ms`, `AppDurations.guessWrongFeedbackDelay = 1500ms` (`lib/core/theme/tokens/app_motion.dart`) surfaced as `MxDurations.guess*` and aliased in `guess_motion.dart`. Footer countdown progress + delayed grade staging both consume the per-grade duration. | **P3**: Long-press card actions (shared with #13/#14). | Domain split lands; option ordering deterministic per `(sessionId, itemId, mode, cardId, shuffleAnswers)` seed. | Covered by `test/domain/study/guess/guess_option_builder_test.dart` (11 cases) and `test/presentation/guess_mode_session_view_test.dart` (799/800 + 1499/1500 boundary pumps). |
...
| 4 | `lib/domain/study/distractor_sampler.dart` | **Resolved 2026-05-31** (Prompt 10): extracted as `lib/domain/study/guess/guess_option_builder.dart`. `GuessOptionBuilder.build` is the single source for Guess option generation; both `guess_mode_session_view.dart` and `studyGuessAnswerOptions` delegate to it. (Match still uses the inline notifier helper, out of Prompt 10 scope.) | ✅ Domain (Guess) |
...
| **P2-4** | ✅ DONE (2026-05-31, Prompt 10) Guess countdown durations verified as named constants (`guessCorrectAdvanceDelay = 800ms`, `guessWrongFeedbackDelay = 1500ms`) and option generation promoted to domain `GuessOptionBuilder`. | #15, §3.13 item 4 | XS |
