...
| --- | --- | --- | --- |
| Library V1 | Partial | Top-level folders, inline scope-local search, true-empty vs no-results, root folder creation. Root-level decks remain Future/Target. | `docs/checklist/implementation-ledger.md` Prompt 18/18B/30B; `docs/wireframes/02-library.md`; `test/presentation/library_overview_screen_test.dart` |
...
| Deck management V1 | Current for verified folder-owned deck flows | Create, rename, move, delete, duplicate/export/import entry through Folder Detail / Flashcard List owners. Root-level deck creation remains Future/Target. | Prompt 19 ledger row; `docs/business/deck/deck-management.md`; folder/flashcard focused tests |
...
| Root-level decks | Blocked / Target | Library V1 is folders-only; Prompt 42 confirmed current production schema/API requires concrete folder ids (`decks.folder_id` non-null, `DeckEntity.folderId` non-null, folder-bound deck create/move/reorder). Prompt 42B added the nullable deck parent migration design in `docs/database/migrations/nullable-deck-parent-migration.md`; root-deck UX/read model/navigation tests remain unpromoted until Prompt 43+ implements migration/code/tests. |
...
| Root-level decks | No Library root-deck read-model/channel or route exposure found. Prompt 42 confirmed `decks.folder_id` is non-null and deck APIs require concrete folder ids. Prompt 42B produced the nullable deck parent migration design, but no schema/code/generated/test implementation has landed. Folder-owned deck flows remain current. | blocked pending Prompt 43 nullable deck parent migration implementation |
