...
| Account Settings V1 | Partial | Google sign-in/sign-out/disconnect, safe auth errors, manual Drive upload/restore. Full restore-protection and account-removal strong-confirm remain Target/Future. | Prompt 22 ledger row; `docs/wireframes/19-settings-account.md`; `test/presentation/settings_screen_test.dart` |
...
| Root-level decks | Future / Target | Library V1 is folders-only; root-deck UX/read model/navigation tests are not promoted. |
| Full restore-protection | Partial / Target | Pre-restore snapshot, Upload local first branch, second destructive confirmation, and abort-on-snapshot-failure remain target scope. |
