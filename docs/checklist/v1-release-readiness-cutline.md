...
| Root-level decks | Blocked / Target | Library V1 is folders-only; Prompt 42 confirmed current production schema/API requires concrete folder ids (`decks.folder_id` non-null, `DeckEntity.folderId` non-null, folder-bound deck create/move/reorder). Root-deck UX/read model/navigation tests are not promoted until nullable deck parent migration design lands. |
...
| Root-level decks | No Library root-deck read-model/channel or route exposure found. Prompt 42 additionally confirmed `decks.folder_id` is non-null and deck APIs require concrete folder ids, so root deck support is blocked on migration/design. Folder-owned deck flows remain current. | blocked pending nullable deck parent migration |
