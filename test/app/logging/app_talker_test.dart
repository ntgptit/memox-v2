...
void main() {
  test(
    'DT1 onInsert: creates a Talker-backed Riverpod observer for local diagnostics',
    () {
  ...
  test(
    'DT1 logEvent: does not attach Riverpod diagnostics outside local-like envs',
    () {
      final config = _testConfig(
      ...
      expect(
        createAppProviderObservers(talker: talker, config: config),
        isEmpty,
      );
    },
  );
