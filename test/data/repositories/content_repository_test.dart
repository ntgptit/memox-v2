...
void main() {
  group('content repositories', () {
    ...
    test('creating a subfolder in an unlocked folder locks it to subfolders', () async {
    ...
    test('deleting the last subfolder resets parent folder mode to unlocked', () async {
      final root = await harness.folderRepository.createRootFolder('Languages');
      final rootId = root.valueOrNull!.id;
      final child = await harness.folderRepository.createSubfolder(
        parentFolderId: rootId,
    ...
    test('moving the last deck resets source folder and preserves flashcard progress', () async {
      final sourceFolder = await harness.folderRepository.createRootFolder('Source');
      final targetFolder = await harness.folderRepository.createRootFolder('Target');
    ...
    test('duplicating a deck copies content and resets progress', () async {
      final folder = await harness.folderRepository.createRootFolder('Languages');
    ...
    test('sort by last studied pushes never-studied flashcards to the end', () async {
      final folder = await harness.folderRepository.createRootFolder('Languages');
      final deck = await harness.deckRepository.createDeck(
      ...
      await (harness.database.update(
        harness.database.flashcardProgress,
      )..where((table) => table.flashcardId.equals(first.valueOrNull!.id))).write(
        const FlashcardProgressCompanion(lastStudiedAt: Value(2000)),
      );
      await (harness.database.update(
        harness.database.flashcardProgress,
      )..where((table) => table.flashcardId.equals(second.valueOrNull!.id))).write(
      ...
      expect(
        list.items.map((item) => item.flashcard.id).toList(),
        <String>[
          second.valueOrNull!.id,
          first.valueOrNull!.id,
    ...
    test('import with mixed valid and invalid lines does not write any flashcards', () async {
      final folder = await harness.folderRepository.createRootFolder('Languages');
      final deck = await harness.deckRepository.createDeck(
        folderId: folder.valueOrNull!.id,
        name: 'Deck A',
    ...
    test('bulk move 200 flashcards stays transaction-safe and preserves progress', () async {
      final sourceFolder = await harness.folderRepository.createRootFolder('Source');
      final targetFolder = await harness.folderRepository.createRootFolder('Target');
