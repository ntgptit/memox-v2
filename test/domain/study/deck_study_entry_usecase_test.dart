...
void main() {
  group('GetDeckStudyEntryUseCase', () {
    test('composes counts and resumable session for a deck scope', () async {
      final repo = _FakeStudyRepo(
        totalCardCount: 42,
        dueCount: 12,
        resumeSessionId: 'session-046',
      );
      final useCase = GetDeckStudyEntryUseCase(repository: repo);

      final entry = await useCase.execute('deck-001');

      expect(entry.totalCardCount, 42);
      expect(entry.dueCount, 12);
      expect(entry.resumeSessionId, 'session-046');
      expect(entry.hasCards, isTrue);
      expect(entry.hasDue, isTrue);
      expect(entry.hasResume, isTrue);
      // Scope probes must use a deck context for this deck id.
      expect(repo.capturedEntryType, StudyEntryType.deck);
      expect(repo.capturedEntryRefId, 'deck-001');
    });

    test('reports an empty study surface when deck has no cards or session', () async {
      final repo = _FakeStudyRepo(
        totalCardCount: 0,
        dueCount: 0,
      ...
      final entry = await useCase.execute('deck-empty');
    ...
    test('cards present but none due hides Today but keeps Study deck', () async {
      final repo = _FakeStudyRepo(
        totalCardCount: 30,
        dueCount: 0,
        resumeSessionId: null,
      ...
      final entry = await useCase.execute('deck-002');
